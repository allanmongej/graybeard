...
// each file shows the same task answered with no skill vs with graybeard, same model.
...
Verbatim model output from a benchmark run — Claude Haiku 4.5, no-skill arm vs graybeard arm, temperature 1, source \`benchmarks/output.json\`. Reproduce: \`npx promptfoo@latest eval -c benchmarks/promptfooconfig.yaml\`.

## Without Graybeard — ${bL} lines of code

${b.response.output.trim()}

## With Graybeard — ${pL} lines of code
...
with no skill (\`## Without Graybeard\`) and with graybeard (\`## With Graybeard\`), so you can
