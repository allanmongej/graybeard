#!/usr/bin/env python3
"""Agentic, multi-file benchmark for graybeard.
...
graybeard: the claude CLI is the harness (already installed, we run inside it). No SDK
...
# tests is still explicitly allowed, so graybeard's "leave a runnable check" discipline is not suppressed.
...
def _selfcheck_split(p: Path):
    ...
    check is the runnable check graybeard's rule asks for -- a positive signal, not source bloat --
...
def code_stats(workdir: Path, selfcheck_as_test: bool = False):
    ...
    reclassified from source to test, so following graybeard's 'leave a runnable check' rule is not
...
def _selftest_plugin_dir():
    ...
    fails = 0
    sentinel = "/tmp/graybeard-selftest-plugin-dir"
    os.environ["GRAYBEARD_PLUGIN_DIR"] = sentinel
    try:
        ok_env = _plugin_dir("graybeard") == sentinel
    finally:
        del os.environ["GRAYBEARD_PLUGIN_DIR"]
    print(f"{'ok ' if ok_env else 'XX '} plugin_dir   env  override honored")
    fails += 0 if ok_env else 1
    missing = "graybeard-does-not-exist-xyz"          # no env, no cache entry -> must sys.exit
    try:
