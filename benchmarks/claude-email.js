// Email under graybeard on Claude (graybeard's primary target), baseline vs graybeard.
const fs = require('fs'), path = require('path');
const { checkPy, pyBlock, TASKS } = require('./robustness-audit.js');
const skill = fs.readFileSync(path.join(__dirname, '..', 'skills', 'graybeard', 'SKILL.md'), 'utf8');
...
(async () => {
  ...
  console.log('model                      baseline   graybeard');
  for (const model of MODELS) {
    const rates = {};
    for (const [arm, sys] of [['baseline', null], ['graybeard', skill]]) {
      let pass = 0, err = 0;
      for (let i = 0; i < N; i++) {
    ...
    console.log(`${model.padEnd(26)} ${rates.baseline.padEnd(10)} ${rates.graybeard}`);
  }
})();
