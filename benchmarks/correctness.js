...
function extractBlocks(text) {
  ...
  // graybeard: terse models often answer with bare, unfenced code. Treat the whole
...
// graybeard: probe once at load; macOS and many Linux images ship python3 only.
let pythonCmd;
function python() {
...
function tmpFile(ext, content) {
  const p = path.join(os.tmpdir(), `graybeard-bench-${Date.now()}-${Math.random().toString(36).slice(2)}${ext}`);
