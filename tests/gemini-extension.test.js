...
// always-on context, commands/*.toml for /graybeard + /graybeard-review, and
...
// carries the load-bearing rules — i.e. if the adapter stops wiring graybeard.
...
const EXTENSION_NAME = 'graybeard';
...
const REUSED_COMMANDS = ['commands/graybeard.toml', 'commands/graybeard-review.toml'];
const REUSED_SKILLS = ['skills/graybeard/SKILL.md'];
// Gemini CLI auto-loads this exact path for extension hooks. Graybeard's
...
test('manifest names the graybeard extension with a pinned version', () => {
...
test('contextFileName resolves to a file carrying the graybeard rules', () => {
  const manifest = loadManifest();
